...
const STRING_NUMBERS = Object.keys(STRING_NUMBERS_MAP)

const numberArray = INPUT.trim()
  .split('\n')
  .map(string => {
    let arrayOfNumbers = []
    let currentindex = 0
    string &&
      string.length &&
      string.split('').reduce((acc, curr, index) => {
        if (curr.match(/(\d+)/)) {
          arrayOfNumbers.push(curr)
          currentindex = index
        } else {
          const subString =
            currentindex !== index
              ? (acc + curr).substring(currentindex, index + 1)
              : ''
          const number = STRING_NUMBERS.find(num => subString.includes(num))
          if (number) {
            arrayOfNumbers.push(STRING_NUMBERS_MAP[number])
            currentindex = index
          }
        }
        return (acc += curr)

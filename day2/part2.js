...
const DEFAULT_BALLS = {red: 0, blue: 0, green: 0}

const BALLS = ['red', 'blue', 'green']
...
INPUT.split('\n').forEach((string, stringIndex) => {
  ...
  string.split(': ')[1].split('; ').forEach(turn => {
    turn.split(', ').forEach((comb) => {
      BALLS.forEach(ball => {
        if (comb.includes(ball)) {
          MAX_BALLS[stringIndex][ball] = Math.max(parseInt(comb.match(/\d/g).join("")), MAX_BALLS[stringIndex][ball])
        }
      })
    })
  })
